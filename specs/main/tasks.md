...
- GET /links: supporte `?archived=true` pour retourner uniquement les liens archivés (permet le compteur "archivés" du dashboard)
- PATCH /links/:id: accepte `{ url?, title?, description? }` — si url change, re-scrape les OG tags; seul le propriétaire peut modifier (403 sinon)
